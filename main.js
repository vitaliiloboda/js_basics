...
function startGame() {
    ...
    barrier_timer = setInterval(createBarrier, 5000); //каждые 500мс запускае фукнцию createBarrier
    ...
    while (!barrierCreated) { //пока еду не создали
        ...
        if (!barrier_cell_classes.includes('snake-unit')) {
            ...
            for (var i = 0; i < barrier_cell_classes.length; i++) {
                classesB += barrier_cell_classes[i] + ' ';
            }

            barrier_cell.setAttribute('class', classesB + 'barrier-unit');
            barriers.push(barrier_cell); // добавляем клетку в массив барьеров
            barrierCreated = true;
        }
    }
}
